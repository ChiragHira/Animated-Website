...
const Navbar = () => {
  ...
  return (
    <div className={`fixed z-999 w-full top-0 py-4 px-8 font-["Neue Montreal"] flex justify-between items-center backdrop-blur-md ${hidden ? '-translate-y-full' : 'translate-y-0'} transition-transform duration-300 ease-in-out`}>
      ...
      {/* Mobile Menu */}
      <div className="sm:flex lg:hidden pr-4">
        <GiHamburgerMenu
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden absolute top-16 left-0 w-full bg-white shadow-lg z-50"
          >
            <motion.div
              initial="hidden"
              animate="visible"
              ...
              {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
                (item, index) => (
                  <motion.a
                    ...
                    className="w-full text-center text-lg font-light capitalize py-2 hover:bg-gray-100 transition"
                    href="#"
                  >
                    {item}
                  </motion.a>
